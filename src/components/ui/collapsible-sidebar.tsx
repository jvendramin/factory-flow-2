...
export function Sidebar({ children, className }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        "h-screen overflow-y-auto border-r border-sidebar-border bg-sidebar transition-all duration-300 ease-in-out",
        collapsed ? "w-16" : "w-64",
        className
      )}
    >
...
export function SidebarSection({ children, title, className }: SidebarSectionProps) {
  ...
  return (
    <div className={cn("py-3", className)}>
      {title && (
        <h3 className={cn(
          "px-4 text-xs font-semibold uppercase tracking-wider text-sidebar-foreground/50 mb-2",
          collapsed && "sr-only"
        )}>
          {title}
        </h3>
